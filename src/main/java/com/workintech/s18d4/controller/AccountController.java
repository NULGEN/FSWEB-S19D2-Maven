...
public class AccountController {
    ...
    public AccountResponse update(@RequestBody Account account, @PathVariable long customerId) {
        ...
        int index0fToBeUpdated = customer.getAccounts().indexOf(toBeUpdatedAccount);
        customer.getAccounts().set(index0fToBeUpdated, account);
        account.setCustomer(customer);
        accountService.save(account);
        return new
                AccountResponse(account.getId(), account.getAccountName(), account.getMoneyAmount(),
